...
int main( int argc, char *argv[] ){
	...
	pthread_t threads[ROW+2];//9 logs + 1 frog + 1 map
	...
	for (log_num = 1; log_num < ROW; log_num++){
		if (log_num == 0){
			...
			if(logthread[log_num] != 0){
				printf("ERROR: return code from log pthread_create() is %d", logthread[log_num]);
				exit(-1);
			}
	...
	if (frogthread != 0){
		printf("ERROR: return code from frog_control pthread_create() is %d", frogthread);
		exit(-1);
	}
	...
	if (mapthread != 0){
		printf("ERROR: return code from map pthread_create() is %d", mapthread);
		exit(-1);
	}
	...
	{	
		...
		if (return_code != 0)
		{
			printf("ERROR: return code from status pthread_join() is %d", return_code);
			exit(-1);
		}
	}
...
void *logs_move( void *t ){
	long lognum = (long)t;
	int log_num = lognum;
	//printf("this is logthrad %d", log_num);
	...
	while(check_gamestatus()){
		...
		if (log_num%2 == 0){//log not singular moves left
			...
			/* system("clear");
			printf("startpos:_________ %d", startpos);
			sleep(5); */
